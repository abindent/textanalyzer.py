...
from textanalyzer.constants import Django
...
def build_absolute_uri(request, location, protocol=None):
    ...
    if request is None:
        ...
        site = Site.objects.filter(id=Django.SITE_ID).first()
        ...
        if not (bits.scheme and bits.netloc):
            ...
            site_obj = Site.objects.filter(id=Django.SITE_ID).first()

            uri = "{protocol}://{domain}{url}".format(protocol=account_settings.DEFAULT_HTTP_PROTOCOL, domain=site_obj.domain, url=location)
